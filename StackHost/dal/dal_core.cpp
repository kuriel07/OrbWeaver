...
dal_conn * dal_create_conn(char * conn_str) {
	char * dbt;
...
//open connection
uint8 dal_open(dal_conn * conn) {
	return conn->backend->open(conn);
}
//close connection
void dal_close(dal_conn * conn) {
...
//query statement
dal_result * dal_query(dal_conn * conn, char * statement) {
	return conn->backend->query(conn, statement);
}
//prepare statement
dal_prepared_statement * dal_prepare(dal_conn * conn, char * name, char * statement) {
	return conn->backend->prepare(conn, name, statement);
}
//execute statement
dal_result * dal_execute(dal_conn * conn, dal_prepared_statement * statement, int count, char * args[]) {
	return conn->backend->execute(conn, statement, count, args);
}
//fetch all result
dal_array * dal_fetch_all(dal_result * result) {
	return ((dal_node *)result)->conn->backend->fetch(result);
}
//get result count
int dal_num_rows(dal_result * result ) {
	return ((dal_node *)result)->conn->backend->count(result);
}
//get first result
dal_array * dal_fetch_first(dal_result * result) {
	return ((dal_node *)result)->conn->backend->first(result);
}

//get next result
dal_array * dal_fetch_next(dal_result * result) {
	return ((dal_node *)result)->conn->backend->next(result);
}

//get result at/key
dal_data * dal_row_at(dal_array * arr, int index) {
	return NULL;
}

dal_data * dal_row_key(dal_array * arr, char * name) {
	return NULL;
}
