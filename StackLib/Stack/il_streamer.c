	...
	switch(opcode) {
		...
		case INS_SYSCALL15:
			...
			if(index < 254) {
				...
				_istream_code_buffer[offset + 1] = 255;
				_istream_code_buffer[offset + 2] = (index / 256);
				_istream_code_buffer[offset + 3] = (uchar)index;
				sprintf(_RECAST(char *, dbgbuf), "\t\t%s_%d %d", _ins_name[opcode], (uint32)opcode & 0x0F, index);
				#if IS_CODEDEBUG
				is_debug(offset, dbgbuf);
				#endif
...
static uint32 is_link_code(uint32 size) {
	...
	while(pc != size) {
		...
		switch(opcode) {
			...
			case INS_SYSCALL14:
			case INS_SYSCALL15:
				if(_istream_code_buffer[pc + 1] < 254) {
					...
					index = ((uint32)_istream_code_buffer[pc + 2] * 256) + _istream_code_buffer[pc + 3];
					printf("syscall %d\n", index);
					sprintf(_RECAST(char *, dbgbuf), "\t\t%s_%d %d", _ins_name[opcode], (uint32)opcode & 0x0F, (uint32)index);
					#if IS_CODEDEBUG
					is_debug(pc, dbgbuf);
					#endif
					as_print(pc, dbgbuf);
...
uint32 is_link_optimize(uchar mode) {
	...
	//pk_set_codebase((pk_object *)pk_get_root(), (uint8 *)_istream_code_size);
	return _istream_code_size;
}

void * is_get_codebuffer() {
	return (void *)_istream_code_buffer;
}
