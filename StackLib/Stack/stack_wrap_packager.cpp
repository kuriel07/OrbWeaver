	...
	while(offset < e_offset) {
		...
		switch(t & 0x1f) {
			...
			case ASN_TAG_IA5STRING:			//valid class sequence
				...
				parent->codebase = codebase;
				break;
			case ASN_TAG_OCTSTRING:		//class method
				//offset += fread(&t, 1, 1, file);		//l
				offset += lk_read(&t, 1, file);
	 			if(t & 0x80) {
					//offset += fread(&l, 1, 2, file);
				...
				buffer[l-3] = 0;
				//printf("method : %s\n", buffer);
				method_obj = (pk_object *)pk_register_method((pk_class *)parent, buffer, numargs, 0, 0, m_offset);
			...
			case ASN_TAG_OBJDESC:		//interface method
				...
				if(l <= 3) return offset;
				//offset += lk_read(_RECAST(uchar *, &m_offset), 2, file);
				offset += lk_read(&call_type, 1, file);
				offset += lk_read(&ret_type, 1, file);
				offset += lk_read(&numargs, 1, file);
				offset += lk_read(args_buffer, numargs, file);
				if(l <= (3 + numargs)) return offset;
				offset += lk_read(buffer, l - (3 + numargs), file);
				buffer[l-3] = 0;
				//printf("method : %s\n", buffer);
				method_obj = (pk_object *)pk_register_method((pk_class *)parent, buffer, numargs, call_type, ret_type, m_offset);
				method_obj->codebase = codebase;
