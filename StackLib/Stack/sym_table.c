...
symrec * st_sym_select_s(uchar type, uchar * name, uint16 length) {
	symrec * iterator = _symhead;
	uint32 stln;
	//uint32 length;
	if(iterator == NULL) return NULL;
	...
	while(iterator != NULL) {							//check for iterator
		if(((iterator->type & 0x7F) & type) != 0) {						//iterate by type
			...
			if(length == iterator->length && memcmp(name, iterator->name, length) == 0) {		//compare name
				//printf("select %s\n", iterator->name);
				return iterator;
			}
		}
...
symrec * st_sym_select(uchar type, uchar * name) {
	return st_sym_select_s(type, name, strlen(_RECAST(const char *, name)));
}
...
symrec * st_sym_create_s(uchar type, uchar * name, uint16 length, uint32 offset) {
	//allocate memory
	symrec * rec = (symrec *)malloc(sizeof(symrec) + length);	
	memset(rec, 0, sizeof(symrec) + length);
	//initialize value
	rec->type = type;
	rec->length = length;
	memcpy(_RECAST(char *, rec->name), _RECAST(const char *, name), rec->length);
	//rec->name[length] = 0;
	rec->offset = offset;
	rec->list = NULL;
	rec->ptotal = 0;
...
symrec * st_sym_create(uchar type, uchar * name, uint32 offset) {
	return st_sym_create_s(type, name, strlen(_RECAST(const char *, name)), offset);
}

symrec * st_sym_create_array(uchar * arrval, uint16 length, uint32 offset) {
	//allocate memory
	symrec * rec = (symrec *)malloc(sizeof(symrec) + length);	
	memset(rec, 0, sizeof(symrec) + length);
	//initialize value
...
symrec * st_sym_insert_s(uchar type, uchar * name, uint16 length, uint32 offset) {
	symrec * rec = st_sym_create_s(type, name, length, offset);
	rec = st_sym_add_record(rec);
	return rec;
}

symrec * st_sym_insert(uchar type, uchar * name, uint32 offset) {
	return st_sym_insert_s(type, name, strlen(_RECAST(const char *, name)), offset);
}

symrec * st_sym_insert_array(uchar * arrval, uint16 length, uint32 offset) {
