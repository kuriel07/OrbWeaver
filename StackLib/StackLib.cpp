...
void StackCompiler::RegisterResource(System::String^ constant, array<System::Byte>^ bytes) {
	uchar * str_constant = (uchar*)(void*)Marshal::StringToHGlobalAnsi(constant);
	pin_ptr<System::Byte> p = &bytes[0];
	lp_update(str_constant, _RECAST(uchar *, p), bytes->Length);
}
...
uint32 vm_fetch(VM_DEF_ARG, uint32 offset, uchar * buffer, uint32 size) {
	...
	memcpy(buffer, _RECAST(uchar *, (uchar *)vm_get_package() + offset), size);
	return size;
}

StackVirtualMachine::StackVirtualMachine() {
	this->vcx = _RECAST(vm_instance *, malloc(sizeof(vm_instance)));
}
...
void StackVirtualMachine::Initialize(array<System::Byte>^ codebase, int offset)
{
	...
	vm_init(this->vcx, NULL, 2048 * 1024);
	ctx = this->vcx;
	vm_set_package(_RECAST(void *, ptr));
}
